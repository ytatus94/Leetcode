# 座標型 DP, 最值型 DP
# 轉移方程:
#   f[i][j] = up[i][j] + down[i][j] + left[i][j] + righr[i][j]
#   up[i][j] = 炸彈放在 (i, j) 往上方炸能炸死多少敵人
#   up[i][j] = up[i-1][j]: (i, j) 是空
#   up[i][j] = up[i-1][j] + 1: (i, j) 是敵人
#   up[i][j] = 0: (i, j) 是牆
#   要分成上下左右分開討論
#   要注意炸的方向不同會使得 i 和 j 是加 1 或減 1 還有計算順序要從上往下還是由下往上，從左往右還是由右往左
# 初始條件:
#   up[0][j] = 0: (0, j) 是空或是牆
#   up[0][j] = 1: (0, j) 是敵人
# TC = O(MN), SC = O(MN)
