...
#   case1. A[i-1] 不是 LCS 的一部分，可能是由 A 的前 i-1 個元素和 B 組成
#   case2. B[j-1] 不是 LCS 的一部分，可能是由 A 和 B 的前 j-1 個元素組成
#   case3. A[i-1] = B[j-1] 必是 LCS 的一部分，看 A[0:i-1] 和 B[0:j-1] 就好
# 初始條件
#   f[0][j>0] = 0
#   f[i>0][0] = 0
#   如果有一個是空字串，那 LCS 就是空
# TC = O(MN), SC = O(MN) 可以用滾動數組優化到 O(N)
